    ...
    try:
        ...
        for item in patient_symptoms:
            ...
            if normalized in normalized_symptoms:
                original_symptom = normalized_symptoms[normalized]
                input_vector[symptoms_dict[original_symptom]] = 1
...
def validate_symptoms(user_input):
    try:
        ...
        cleaned = [s.strip() for s in user_input.split(',')]
        
        for symptom in cleaned:
            normalized = symptom.lower().replace(' ', '_')
            
            if normalized in normalized_symptoms:
                valid.append(normalized_symptoms[normalized])
                continue
            
            close_matches = get_close_matches(normalized, normalized_symptoms.keys(), n=3, cutoff=0.6)
...
def home():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('index.html', 
                                  message=f"System error: {str(e)}. Please try again with different symptoms.")
